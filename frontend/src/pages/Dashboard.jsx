...
import { ArrowDown, BarChart2, ChevronDown, ChevronUp, DollarSign, PiggyBank, Plus, ShoppingCart, TrendingDown, TrendingUp, TrendingUp as ProfitIcon, PieChart as PieChartIcon, Wallet } from 'lucide-react'
import { useOutletContext } from 'react-router-dom'
import { getTimeFrameRange, getPreviousTimeFrameRange, calculateData } from '../components/Helper.jsx'
import Financial from '../components/FinancialCard.jsx'
...
const Dashboard = () => { 
  ...
  // Custom label renderer to place labels outside the donut with connector lines
  const renderDonutLabel = (entry) => {
    const { cx, cy, midAngle, outerRadius, percent, name } = entry;
    const RADIAN = Math.PI / 180;
    const radius = outerRadius + 18; // distance from center to label
  ...
  return (
    <div className={dashboardStyles.container}>
      ...
      <div className={dashboardStyles.summaryGrid}>
         ...
         } label={`${timeFrameRange.label} Savings`} value={`$${Math.round(displayIncome - displayExpenses).toLocaleString()}`}
         additionalContent={
      ...
      <div className={`${dashboardStyles.pieChartContainer} min-w-0`}>
